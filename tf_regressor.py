...
class Regressor(tf.keras.Model):
    ...
    def add_f(self, f, dim_w=None, w0=None, b0=None, random_state=None, opt='sgd'):
        ...
        try:
            self.w = (keras.Variable(w0.astype('float'), dtype='float32') if w0 is not None
                      else keras.Variable(np.random.randn(dim_w), dtype='float32'))
            ...
            self.w = keras.Variable([float(w0)], dtype='float32')
        ...
        # Configura el bias
        self.b = (keras.Variable(float(b0), dtype='float32') if b0 is not None
                  else keras.Variable(np.random.rand(), dtype='float32'))
